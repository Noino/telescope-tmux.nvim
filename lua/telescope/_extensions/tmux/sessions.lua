...
local sessions = function(opts)
    ...
    local current_session = tutils.get_os_command_output({ 'tmux', 'display-message', '-p', tmux_commands.session_id_fmt })
        [1]
    ...
    local custom_actions = transform_mod({
        ...
        delete_session = function(prompt_bufnr)
            ...
            local confirmation = string.lower(vim.fn.input("Kill session '" .. session_display .. "'? [Y/n] "))
            if confirmation ~= 'y' and confirmation ~= '' then return end
    ...
    pickers.new(opts, {
        ...
        attach_mappings = function(prompt_bufnr, map)
            ...
            map('i', '<M-a>', custom_actions.create_new_session)
            map('n', '<M-a>', custom_actions.create_new_session)
            map('i', '<M-d>', custom_actions.delete_session)
            map('n', '<M-d>', custom_actions.delete_session)
            map('i', '<M-r>', custom_actions.rename_session)
            map('n', '<M-r>', custom_actions.rename_session)
